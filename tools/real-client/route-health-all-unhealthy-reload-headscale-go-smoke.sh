...
route="${REAL_CLIENT_ROUTE:-10.94.0.0/24}"

route_health_all_unhealthy_reload_policy() {
  ruby -rjson -e '
    route = ARGV.fetch(0)
...
initial_policy="$(route_health_all_unhealthy_reload_policy tag:router-a tag:router-b)"
reload_policy="$(route_health_all_unhealthy_reload_policy tag:router-a tag:router-b)"

HEADSCALE_GO_VERSION="${HEADSCALE_GO_VERSION:-${HEADSCALE_GO_CURRENT_VERSION}}" \
REAL_CLIENT_WORKDIR="${REAL_CLIENT_WORKDIR:-target/real-client/route-health-all-unhealthy-reload-headscale-go-smoke}" \
...
REAL_CLIENT_EXPECT_APPROVED_ROUTES_BY_CLIENT="${REAL_CLIENT_EXPECT_APPROVED_ROUTES_BY_CLIENT:-${route};${route}}" \
