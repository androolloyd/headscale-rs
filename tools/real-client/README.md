...
| Lifecycle | `authkey-relogin-different-user` | `authkey-relogin-different-user-smoke.sh` | `authkey-relogin-different-user-headscale-go-smoke.sh` | Auth-key logout then different-user relogin creates a new node and preserves the old node |
...
| Database | `postgres-authkey-relogin-different-user` | `postgres-authkey-relogin-different-user-smoke.sh` | `postgres-authkey-relogin-different-user-headscale-go-smoke.sh` | Production Postgres auth-key logout then different-user relogin creates a new node and preserves the old node |
...
| Database | `postgres-derp-native` | `postgres-derp-native-smoke.sh` | `postgres-derp-private-headscale-go-smoke.sh` | Production Postgres native DERP relay, STUN, relay path, native raw verify-client admissions, and DERP map metadata |
| Database | `postgres-derp-native-websocket` | `postgres-derp-native-websocket-smoke.sh` | `postgres-derp-native-websocket-headscale-go-smoke.sh` | Production Postgres native DERP-over-WebSocket relay path, WebSocket verify-client admissions, status health, and DERP map metadata; headscale-go script is an explicit no-equivalent skip |
| Database | `postgres-derp-native-reload` | `postgres-derp-native-reload-smoke.sh` | `postgres-derp-native-reload-headscale-go-smoke.sh` | Production Postgres native DERP map stability, raw verify-client admissions, relay path, and status health after live policy reload; headscale-go script is an explicit no-equivalent skip |
| Database | `postgres-derp-native-restart` | `postgres-derp-native-restart-smoke.sh` | `postgres-derp-native-restart-headscale-go-smoke.sh` | Production Postgres native DERP stock clients reconnect, prove raw verify-client admissions, and relay after Rust server restart; headscale-go script is an explicit no-equivalent skip |
...
| Addresses | `prefix-family-v4-to-dual-backfill` | `prefix-family-v4-to-dual-backfill-smoke.sh` | `prefix-family-v4-to-dual-backfill-headscale-go-smoke.sh` | IPv4-to-dual-stack backfill plus post-backfill restart hydration after prefix migration |
| Addresses | `prefix-family-dual-stack-to-ipv4-only-backfill` | `prefix-family-dual-stack-to-ipv4-only-backfill-smoke.sh` | `prefix-family-dual-stack-to-ipv4-only-backfill-headscale-go-smoke.sh` | Dual-stack-to-IPv4-only backfill plus post-backfill restart hydration after prefix-family removal |
| Addresses | `prefix-family-dual-stack-to-ipv6-only-backfill` | `prefix-family-dual-stack-to-ipv6-only-backfill-smoke.sh` | `prefix-family-dual-stack-to-ipv6-only-backfill-headscale-go-smoke.sh` | Dual-stack-to-IPv6-only backfill plus post-backfill restart hydration after prefix-family removal |
...
The same-user relogin wrappers run an initial auth-key login, mint a fresh key
for the same user, run `tailscale logout`, and relogin with the existing
client state. They assert stable Tailscale IPs and stable logical node state.
...
asserts the stock client logs in as that user while the previous user's node is
preserved and a new node is created.
...
- `REAL_CLIENT_OIDC_POLICY_CHURN_VIEWER_USER`,
  ...
  viewer hostname, and OIDC peer hostname for policy-churn runs.
- `HEADSCALE_GO_VERSION` defaults to the release in
  `tools/real-client/headscale-go-baseline.sh`.
- `HEADSCALE_GO_BIN` can point at an existing `headscale` binary.
