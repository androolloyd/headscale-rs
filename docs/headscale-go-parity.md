...
| State manager | Partial | Registry generation watches, policy/DNS wake-ups, ephemeral GC, route primary-state updates, and policy-update route auto-approval fan-out have local coverage; still match headscale-go node update batching, full state-change reason sets, route update propagation, and map debug state. |
