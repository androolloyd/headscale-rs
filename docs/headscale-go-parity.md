...
| Metrics/debug | Partial | `/debug/overview`, `/debug/routes`, `/debug/derp`, `/debug/registration-cache`, `/debug/filter`, `/debug/policy`, `/debug/ssh`, `/debug/nodestore`, `/debug/mapresponses`, `/debug/batcher`, and `/debug/policy-manager` now cover the headscale-go debug surface for state overview, primary routes, DERP map inspection, registration-cache metadata, current packet filters, raw policy output, per-node SSH policy output, NodeStore summaries, the disabled map-response dump state, active streaming map connections, and policy-manager inspection; Rust runtime JSON snapshots are still not a byte-for-byte persisted `types.Node`/`types.Config` dump. Still match `/debug/config`, Prometheus metrics, pprof/statviz-compatible surfaces where applicable, and operational logs that tests depend on. |
