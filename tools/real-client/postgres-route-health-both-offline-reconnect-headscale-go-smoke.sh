...
REAL_CLIENT_RESTART_ROUTE_HEALTH_BOTH_OFFLINE_RECONNECT=true \
REAL_CLIENT_ROUTE_HEALTH_NO_RESTART=true \
REAL_CLIENT_WORKDIR="${REAL_CLIENT_WORKDIR:-target/real-client/postgres-route-health-both-offline-reconnect-headscale-go-smoke}" \
REAL_CLIENT_TIMEOUT_SECS="${REAL_CLIENT_TIMEOUT_SECS:-320}" \
REAL_CLIENT_RESTART_ROUTE="${REAL_CLIENT_RESTART_ROUTE:-10.94.0.0/24}" \
