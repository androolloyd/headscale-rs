...
| Policy and health gRPC slice | Partial | `grpc::upstream_tests::*`, `headscale-api/tests/grpc_gateway_e2e.rs`, and `headscale-db::policies` cover upstream `GetPolicy`, `SetPolicy`, and `Health` RPCs backed by `PolicyStore`, preserving raw HUJSON and update timestamps across both tonic and grpc-gateway `/api/v1/policy`/`/api/v1/health`; health now pings a wired database health probe/SQLite pool and surfaces headscale-go-style ping failures through gRPC and grpc-gateway. DB-mode policy persistence now appends Go-shaped `policies` rows and reads the latest non-deleted row when a policy pool is wired. Exact file-mode policy behavior and policy-mode enforcement remain open |
