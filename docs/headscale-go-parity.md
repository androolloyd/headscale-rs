...
| Poll/map runtime | Partial | Match `/machine/{key}/map`, streaming updates, keep-alives, full vs incremental map responses, peer changes, endpoint updates, disco key updates, hostinfo updates, logout, and expiry behavior; map node FQDNs now follow the configured DNS base domain, and loaded policies now reduce visible peers using symmetric node/route access checks, leaving downstream Octra domains to Octra-side wiring. |
...
| Peer map/matchers | Partial | `headscale-api/tests/policy_consolidation.rs` covers a Rust `build_peer_map_for_doc` equivalent for symmetric one-way ACL visibility and route-targeted subnet-router visibility, and `tailscale_wire::map::tests::map_response_*policy*` verifies loaded policies reduce `/map` peers while preserving route-visible routers. Still add exact differential harness coverage against headscale-go `PolicyManager.BuildPeerMap`/`MatchersForNode`, autogroup:self matcher cache behavior, and per-node packet-filter reduction. |
