...
if [[ -z "${headscale_go_tls}" ]]; then
  if ((do_reauth_after_login || authkey_relogin_requested_flag)); then
...
if [[ -n "${dns_search_domains_json}" ]]; then
  ruby -rjson -e '
    puts "  search_domains:"
    JSON.parse(ARGV.fetch(0)).each do |domain|
      abort("DNS search domain must be a string: #{domain.inspect}") unless domain.is_a?(String)
      puts "    - #{domain.to_json}"
    end
  ' "${dns_search_domains_json}" >>"${config_path}"
else
...
if ((authkey_relogin_requested_flag)); then
  ...
  for idx in "${!client_names[@]}"; do
    ...
    if ((authkey_relogin_expired_flag)); then
      authkey_id="$(ruby -rjson -e 'j=JSON.parse(File.read(ARGV.fetch(0))); id = j["id"] || j["ID"]; abort("missing preauth key ID") unless id; puts id' "${work_dir}/preauth-relogin-${idx}.json")"
      "${headscale_bin}" -c "${config_path}" -o json preauthkeys expire \
        --id "${authkey_id}" \
        >"${work_dir}/preauth-relogin-${idx}-expired.json"
    fi
  ...
  if ((authkey_relogin_expired_flag)); then
    ...
    ' "${work_dir}/nodes-before-relogin.json" "${work_dir}/nodes-after-relogin.json" "${expected_machine_count}" "expired"
    echo "::endgroup::"
...
if [[ -n "${policy_reload_json}" ]]; then
  if [[ -z "${policy_json}" ]]; then
    echo "REAL_CLIENT_POLICY_RELOAD_JSON requires REAL_CLIENT_POLICY_JSON so headscale-go starts with a policy file" >&2
    exit 2
  fi
  ...
  if ! (
    "${headscale_bin}" -c "${config_path}" -o json nodes list >"${work_dir}/nodes-after-route-health-all-unhealthy.json" &&
      ruby -rjson -e '
        ...
        abort("all-unhealthy fallback did not retain last known primary for #{route}: got #{after_owner}, expected #{first_owner}") unless after_owner == first_owner
