...
route_health_mixed_exit_reload_policy() {
  ruby -rjson -e '
    route = ARGV.fetch(0)
...
initial_policy="$(route_health_mixed_exit_reload_policy tag:router-a)"
reload_policy="$(route_health_mixed_exit_reload_policy tag:router-a tag:router-b)"

HEADSCALE_GO_VERSION="${HEADSCALE_GO_VERSION:-4483fd0cad38717913e7509fc50f9d48c691b02b}" \
REAL_CLIENT_WORKDIR="${REAL_CLIENT_WORKDIR:-target/real-client/route-health-mixed-exit-reload-headscale-go-smoke}" \
