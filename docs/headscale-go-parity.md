...
| API-key persistence, admin REST auth, CLI, and gRPC slice | Partial | `headscale-db::api_keys` unit tests, `headscale-api/tests/api_keys_e2e.rs`, `headscale-cli/tests/admin_client.rs`, and `grpc::upstream_tests::*` cover headscale-go key shape, Go-compatible `api_keys.hash`/datetime row layout, bcrypt-secret storage, legacy rows, prefix parsing, validate/expire/delete/list, bearer auth, CLI HTTP wiring, the four upstream `HeadscaleService` API-key RPCs, opt-in gRPC bearer API-key auth, and a tonic service mount helper |
| Pre-auth-key persistence, admin REST/CLI, and gRPC slice | Partial | `headscale-db::preauth_keys` unit tests, `headscale-api/tests/preauth_e2e.rs`, `headscale-cli/tests/admin_client.rs`, and `grpc::upstream_tests::*` cover create/list/expire/delete, headscale-go `hskey-auth-<12>-<64>` token shape, Go-compatible `pre_auth_keys.prefix/hash/used` row layout, nullable numeric `user_id` storage when a persistent user resolver is wired, reusable/ephemeral/used/tags, tags-only keys, ID-based upstream gRPC RPCs, and tag/user validation; exact upstream error text and registration use of persisted node FKs remain open |
| Node gRPC slice | Partial | `proto/node.proto` now matches the upstream `headscale.v1.Node` message/RPC schema instead of the previous Octra mesh schema, and `grpc::upstream_tests::*` covers `ListNodes`, `GetNode`, `SetTags`, `SetApprovedRoutes`, `BackfillNodeIPs`, `DebugCreateNode`, `RegisterNode` from the debug registration cache, `RenameNode`, `ExpireNode`, and `DeleteNode`; `headscale-db::headscale_nodes` covers the Go-shaped `nodes` row layout/core node mutations, and `PersistentMachineAdmin` now lets admin/gRPC node operations expose numeric `nodes.id` values from SQLite. Route responses preserve headscale-go sticky primary-route ownership and keep exit routes out of `PrimaryRoutes` while surfacing them as serving routes where upstream does. Runtime map/registration still use the current wire registry by default, and web/OIDC registration-cache integration remains open |
| Policy and health gRPC slice | Partial | `grpc::upstream_tests::*` covers upstream `GetPolicy`, `SetPolicy`, and `Health` RPCs backed by `PolicyStore`, preserving raw HUJSON and update timestamps; exact file-mode policy behavior, DB-mode persistence, gRPC-gateway HTTP annotations, and DB ping failure semantics remain open |
...
| Database models | Partial | Match users, nodes, preauth keys, API keys, routes, policies, migrations, soft-delete behavior, timestamps, expiry, and uniqueness constraints; API-key, pre-auth-key, user, and canonical node row shapes now track headscale-go, legacy Octra mesh nodes are isolated in `octra_nodes`, admin/gRPC users and nodes can use Go-compatible SQLite stores, and preauth rows can store numeric user IDs. Route state/policy persistence and the live wire runtime are still not fully DB-backed. |
| Node registration | Partial | Match preauth-key registration, reusable/ephemeral/expired keys, CLI debug registration, web registration, OIDC registration, machine-key reuse, node-key expiry, forced tags, and duplicate handling; pre-auth key token/storage format plus user/node storage shapes now match upstream, and CLI debug registration can persist through the Go-shaped node adapter, but real wire registration still uses the current Rust machine registry and web/OIDC registration are not yet wired through the persisted node FK model. |
| Poll/map runtime | Partial | Match `/machine/{key}/map`, streaming updates, keep-alives, full vs incremental map responses, peer changes, endpoint updates, disco key updates, hostinfo updates, logout, and expiry behavior; map node FQDNs now follow the configured DNS base domain, leaving downstream Octra domains to Octra-side wiring. |
| Tailcfg wire model | Partial | `MapRequest` session/keepalive/debug fields, broad `MapNode` metadata including tags/routes/capabilities/CapMap and WireGuard-only metadata, nodeAttrs-to-CapMap emission, `MapResponse.UserProfiles`, peer deltas, packet-filter deltas, health/display/debug/control messages, and `SSHPolicy` are covered at the JSON parity layer; still wire these into real runtime streaming behavior, full route state, and client capability negotiation. |
...
| Peer map/matchers | Missing | Port and test `BuildPeerMap`, `MatchersForNode`, route-aware peer visibility, and policy-driven peer reduction. |
...
| Routes | Partial | Advertised and approved route storage now feeds node gRPC responses and map response `AllowedIPs`/primary routes, with stateful headscale-go-style sticky primary promotion/failover and exit routes separated from subnet `PrimaryRoutes`; still match enabled state, full auto-approval integration, API/CLI state changes, and persistence. |
...
| API keys | Partial | DB-backed create/list/expire/delete, headscale-go `hskey-api-<12>-<64>` shape, Go-compatible `api_keys.hash`/datetime row layout, bcrypt secret hashing, legacy `prefix.secret` validation, REST bearer-auth middleware, `headscale apikeys` CLI, upstream `HeadscaleService` API-key RPCs, and opt-in gRPC bearer API-key auth are covered; still add exact grpc-gateway timestamp/status/error parity. |
| Upstream gRPC API | Partial | The `User`, `Node`, `ApiKey`, `PreAuthKey`, `Policy`, and `Health` proto messages plus their implemented `headscale.v1.HeadscaleService` RPCs are generated and backed by admin state, with users/nodes/API keys/preauth keys able to use Go-compatible SQLite adapters; debug-created CLI registration, node route operations, numeric SQLite node IDs, auth-required service mode, a tonic mount helper, and the first grpc-gateway-backed `Health`/`User` HTTP annotation slice are covered; debug/version operations, the remaining grpc-gateway HTTP annotations, external listener wiring, and broader integration tests remain open. |
| REST/swagger API | Partial | Match `/api/v1`, `/swagger`, `/version`, auth, request/response shapes, status codes, and error bodies; the public wire router now covers `/robots.txt`, `/health`, `/version`, `/windows`, `/apple`, `/apple/{platform}`, `/swagger`, `/swagger/v1/openapiv2.json`, `/favicon.ico`, and blank fallback with config-backed helper URLs where upstream uses `server_url`. `grpc_gateway::router` now covers `/api/v1/health` and the singular upstream `/api/v1/user` create/list/rename/delete paths with API-key bearer auth and protojson-style status bodies; remaining `/api/v1` annotations for preauth keys, API keys, nodes, policy, and debug remain open. |
| CLI parity | Partial | Users, nodes, preauthkeys, apikeys, policy, and tailnet have HTTP-admin wrappers and tests; still match upstream command groups, aliases, output formats, config/env handling, remote gRPC mode, and externally visible error text. |
...
| Embedded DERP | Partial | Match DERP server, DERP map loading/defaults, verify endpoint, STUN behavior, private DERP config, TLS behavior, and integration coverage. |
...
| State manager | Missing | Match notifier behavior, node update batching, policy update fan-out, ephemeral GC, route updates, and map debug state. |
| Metrics/debug | Partial | Match Prometheus metrics, debug endpoints, pprof-compatible surfaces where applicable, and operational logs that tests depend on. |
