...
base_domain="${REAL_CLIENT_BASE_DOMAIN:-auth-map-churn.test}"
...
policy_reload_json="${REAL_CLIENT_RELOAD_POLICY_JSON:-$(ruby -rjson -e 'puts JSON.pretty_generate({acls: [{action: "accept", src: ["alice@", "bob@"], dst: ["alice@:*", "bob@:*"]}]})')}"

REAL_CLIENT_WORKDIR="${REAL_CLIENT_WORKDIR:-target/real-client/postgres-web-register-policy-config-map-churn-restart-headscale-go-smoke}" \
