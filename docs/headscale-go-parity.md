...
| Metrics/debug | Partial | `/debug/overview`, `/debug/routes`, `/debug/derp`, `/debug/registration-cache`, `/debug/filter`, `/debug/policy`, `/debug/ssh`, `/debug/mapresponses`, and `/debug/batcher` now mirror headscale-go debug shapes for state overview, primary routes, DERP map inspection, registration-cache metadata, current packet filters, raw policy output, per-node SSH policy output, the disabled map-response dump state, and active streaming map connections; still match remaining debug endpoints, Prometheus metrics, pprof-compatible surfaces where applicable, and operational logs that tests depend on. |
