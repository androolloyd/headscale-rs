...
| Routes | Partial | Advertised and approved route storage now feeds node gRPC responses and map response `AllowedIPs`/primary routes, with stateful headscale-go-style sticky primary promotion/failover, exit routes separated from subnet `PrimaryRoutes`, and registration/map paths applying policy `autoApprovers` without removing existing approvals; still match route enabled-state persistence, policy-change re-approval fan-out, and full API/CLI state changes. |
