...
work_tmp="$(mktemp -d "${TMPDIR:-/tmp}/headscale-rs-postgres-derp-private.XXXXXX")"
...
REAL_CLIENT_WORKDIR="${REAL_CLIENT_WORKDIR:-target/real-client/postgres-derp-private-smoke}" \
...
REAL_CLIENT_RUST_DERP_PORT="${derp_port}" \
...
REAL_CLIENT_RUST_DERPER_BINARY="${derper_bin}" \
REAL_CLIENT_RUST_DERPER_LISTEN_ADDR="0.0.0.0:${derp_port}" \
REAL_CLIENT_RUST_DERPER_CERT_MODE=manual \
REAL_CLIENT_RUST_DERPER_CERT_DIR="${cert_dir}" \
