...
| Policy and health gRPC slice | Partial | `grpc::upstream_tests::*` and `headscale-api/tests/grpc_gateway_e2e.rs` cover upstream `GetPolicy`, `SetPolicy`, and `Health` RPCs backed by `PolicyStore`, preserving raw HUJSON and update timestamps across both tonic and grpc-gateway `/api/v1/policy`/`/api/v1/health`; health now pings a wired database health probe/SQLite pool and surfaces headscale-go-style ping failures through gRPC and grpc-gateway. Exact file-mode policy behavior and DB-mode policy persistence remain open |
