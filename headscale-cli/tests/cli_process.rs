...
fn generate_private_key_unknown_flags_honor_output_format_like_current_upstream() {
    assert_stderr_snapshot(
        ...
        1,
        include_str!("snapshots/utility_generate_private_key_unknown_flag_json.stderr"),
    );
    assert_stderr_snapshot(
        &["gen", "private-key", "-o", "json-line", "--bad"],
        1,
        include_str!("snapshots/utility_generate_private_key_unknown_flag_json_line.stderr"),
    );
    assert_stderr_snapshot(
        &["generate", "private-key", "-oyaml", "--bad"],
        1,
        &format!(
            "{}\n",
            include_str!("snapshots/utility_generate_private_key_unknown_flag_yaml.stderr")
        ),
    );
    assert_stderr_snapshot(
        &["generate", "private-key", "--output", "weird", "--bad"],
        1,
        include_str!("snapshots/utility_generate_private_key_unknown_flag_unknown_output.stderr"),
    );
}
...
fn utility_top_level_unknown_commands_match_upstream_suggestions() {
    for (args, expected) in [
        ...
        assert_stderr_snapshot(args, 1, &format!("{expected}\n"));
    }
}
