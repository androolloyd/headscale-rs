...
| Database | `postgres-derp-native-reload` | `postgres-derp-native-reload-smoke.sh` | `postgres-derp-native-reload-headscale-go-smoke.sh` | Production Postgres native DERP map stability, relay path, and status health after live policy reload; headscale-go script is an explicit no-equivalent skip |
| Database | `postgres-derp-native-restart` | `postgres-derp-native-restart-smoke.sh` | `postgres-derp-native-restart-headscale-go-smoke.sh` | Production Postgres native DERP stock clients reconnect and relay after Rust server restart; headscale-go script is an explicit no-equivalent skip |
