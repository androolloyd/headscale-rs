...
| Metrics/debug | Partial | `/debug/routes` and `/debug/derp` now mirror headscale-go text-vs-JSON negotiation and state shapes for primary routes and DERP map inspection; still match remaining debug endpoints, Prometheus metrics, pprof-compatible surfaces where applicable, and operational logs that tests depend on. |
