...
region_id="${REAL_CLIENT_DERP_REGION_ID:-942}"
region_code="${REAL_CLIENT_DERP_REGION_CODE:-native-restart}"
region_name="${REAL_CLIENT_DERP_REGION_NAME:-headscale-rs native DERP restart}"
...
REAL_CLIENT_WORKDIR="${REAL_CLIENT_WORKDIR:-target/real-client/postgres-derp-native-restart-smoke}" \
...
REAL_CLIENT_DERP_RESTART_AFTER_ASSERTIONS=true \
